...
namespace game {
	...
	base::ReturnCode BodyPart::setEquippedItem(Item *item, bool forceEquip)
	{
	...
	{
		base::Handle<data::Table> dataMap(data::Table::checkDataType(data, "body part"));
		if (!dataMap)
		{
			return false;
		}

		base::Handle<data::String> str(dataMap->at<data::String>("partName"));
		if (str)
		{
			mName = str->string();
		}
		...
		base::Handle<data::Boolean> boo(dataMap->at<data::Boolean>("isWeaponPart"));
		if (boo)
		{
		...
		base::Handle<data::IData> tempData(dataMap->at("equippedItem"));
		if (tempData)
		{
