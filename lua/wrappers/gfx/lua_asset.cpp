...
namespace gfx {
	...
	int Asset_texture(lua_State *lua)
	{
		...
		if (asset)
		{
			...
			if (args == 1)
			{
				base::Handle<Texture> texture = asset->getTexture();
				if (texture)
				{
			...
			else if (args == 2)
			{
				...
				if (lua_isnum(lua, 2))
				{
					base::Handle<Texture> texture = asset->getTexture(lua_tointeger(lua, 2));
					if (texture)
					{
						wrapRefObject<Texture>(lua, texture);
						return 1;
				...
				if (lua_isstr(lua, 2))
				{
					base::ReturnCode result = GfxEngine::getEngine()->getTexture(lua_tostring(lua, 2), texture);
				...
				if (texture)
				{
					...
					lua_pushvalue(lua, 1);
					lua_pushinteger(lua, base::SUCCESS);
					return 2;
				}
			}
